...
from asr2k.lm.pronunciation import prep_pronunciation_from_vocab, prep_vocab_from_text
from asr2k.lm.lexicon import prep_lexicon
from asr2k.lm.grammar import prep_grammar_from_ngram, prep_ngram_from_text
from asr2k.lm.compile import compile_HLG
import torch

def prep_text_lm(lang_dir, lang_id, ngram=3, force=False):

    if not force and Path(f"{lang_dir}/HLG{ngram}.pt").exists():
        return

    lang_dir = Path(lang_dir)

    print("step 0: prep ngram and vocab")
    prep_ngram_from_text(lang_dir, ngram=ngram, force=force)
    prep_vocab_from_text(lang_dir)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('prep text lm')
    parser.add_argument('--lang_dir', help='lang dir')
    parser.add_argument('--lang_id', help='lang id')
    parser.add_argument('--ngram', default=3, type=int, help='ngram')
    ...
    prep_text_lm(lang_dir, lang_id, ngram, force=args.force)